...
public class ProcessImp extends Thread {
  ...
  public void notificarAutorizacaoAcessoSC() throws IOException {
    byte[] message = (MENSAGEM_AUTORIZACAO_ACESSO_SC).getBytes();
    DatagramPacket packet = new DatagramPacket(message, message.length,
        InetAddress.getByName(IP), PORT);
    // send packet
    this.socket.send(packet);
  ...
  public void liberarSC() throws IOException {
    ...
    // notificarLiberacaoOutrosProcessos();
  }
  ...
  private void atualizarParesConectados(String received) throws IOException{
    ...
    if(received.contains("" + this.id) || processosQueJaAvisaramEntrada.contains(received)) return;
    
    processosQueJaAvisaramEntrada.add(received);
    processosQueJaAvisaramEntrada.forEach(processo -> { System.out.println(processo);});
  ...
  public void run() {
    ...
    while (true) {
      ...
      try {
        ...
        if(received.equals(MENSAGEM_AUTORIZACAO_ACESSO_SC) && this.status == StatusType.WANTED && this.solicitouAcessoSC){
